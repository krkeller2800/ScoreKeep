...
The current named reusable-team creation path is `TeamView`: it collects `teamName`, `coachName`, and `teamInfo`; checks duplicate names with a fetch on `Team.name`; creates `Team(name: teamName, coach: coachName, details: teamInfo)`; calls `modelContext.insert(theTeam)`; clears form fields before save; and calls `try? self.modelContext.save()`. Save errors are swallowed. The route uses the shared SwiftUI environment context.

Additional production team-creation-like helpers create blank teams and immediately navigate or select them: `ContentView`, `TeamContentView`, `ScoreContentView`, `EditGameView`, and `PasteView`. These helpers insert `Team(name: "", coach: "", details: "")`, save with optional error handling, and then use the live model for edit or selection. They represent edit/setup/import-adjacent workflows, not the same bounded named simple-team route.

`EditTeamView` edits a bound `Team`, can mutate `name`, `coach`, `details`, and `logo`, checks duplicate names, and deletes the team on disappear when duplicate or blank. Player addition from the team edit view inserts `Player` records. Team deletion can delete related players and save. These edit, media, roster, and delete behaviors remain outside the bounded adapter review.
...
On the supported isolated success path, it performs one `Team` insert, one explicit save, creates a fresh reload context from the same container, fetches by team identity, requires exactly one matching team, verifies fields, verifies nil logo and empty relationships, interprets persisted evidence through canonical mapping, and returns a shallow deterministic transaction result. The adapter remains non-routed and always reports `routingRemainsDisabled`.

<!-- MARK: - 5. Production Call-Site Comparison -->
## 5. Production Call-Site Comparison

Compatible boundary: the named `TeamView` form collects the same supported display fields: name, coach, and details. The current `Team` initializer can represent stable identity, display fields, empty players, empty games, and nil logo. A future bounded route could use this form as the narrow simple-team path.

Mismatches: production currently creates random team identity inside `Team`; production duplicate detection is name-based rather than stable-identity-based; production clears form state before save proof; production swallows save failure; production uses a shared environment context; production has no operation identity; production has no durable retry evidence; blank-team helpers support edit workflows that the adapter rejects because blank names are invalid; logo, roster, game relationships, edit cleanup, and deletion are outside adapter scope.
...
Production call-site compatibility: Partially satisfied. Repository evidence: `TeamView` named form maps to name, coach, details; blank helper routes and `EditTeamView` do not. Test evidence: focused review tests assert legacy references and non-routing. Missing evidence: routed request construction, source classification, UI outcome mapping. Blocking consequence: production route cannot be wired safely yet. Required next action: define one bounded named-team route.
...
One writer: Partially satisfied. Repository evidence: exact legacy write is `TeamView` plus action inserting `theTeam` and saving; alternate blank-team helpers are separate routes; cutover manifest still says team creation current authority is legacy SwiftData. Test evidence: focused tests assert legacy writer references and adapter absence from production sources. Missing evidence: implemented route bypass, no-fallback failure handling, repeated-submit operation identity reuse. Blocking consequence: production routing remains prohibited. Required next action: later task must replace exactly the `TeamView` named-team write with no legacy fallback.
...
The exact future bounded routing seam is the `TeamView` plus action for the named reusable-team form. At that point the legacy `Team(name: teamName, coach: coachName, details: teamInfo)`, `modelContext.insert(theTeam)`, form clear, and `try? self.modelContext.save()` would be bypassed. The adapter result, and only the adapter result, would control whether the form clears, whether success is shown, whether review is needed, and whether retry is allowed.

No fallback legacy write may follow adapter failure. Repeated actions must reuse or preserve the same operation identity until the first request reaches a final disposition. Blank-team helper routes in `ContentView`, `TeamContentView`, `ScoreContentView`, `EditGameView`, and `PasteView` are explicitly outside this seam.
