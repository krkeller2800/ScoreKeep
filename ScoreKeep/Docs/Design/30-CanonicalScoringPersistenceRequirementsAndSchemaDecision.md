...
<!-- MARK: - 40. Task 7.7B Versioned Implementation Record -->
## 40. Task 7.7B Versioned Implementation Record

Task 7.7B implements the approved durable Legacy scoring-operation evidence foundation. The new schema is `ScoreKeepProposedVersionedSchema.V4` with version identifier `4.0.0`. V1 remains unchanged at six Legacy baseball models, V2 remains unchanged at those six plus `TeamCreationOperationEvidenceRecord`, and V3 remains unchanged at exactly 12 models. V4 contains all V3 responsibilities plus exactly one additional responsibility, `LegacyScoringOperationEvidenceRecord`, for a final count of 13 models.

The new record stores only operation evidence for the Legacy scoring writer: stable logical operation identity, deterministic request fingerprint, target game identity, target at-bat identity, submission family, accepted result classification, accepted outcome reference, disposition, conflict evidence, completion state, and evidence schema version. It deliberately excludes UI state, navigation state, display labels, analytics, canonical event history, relationships to mutable baseball facts, and mutable baseball projections. It is not a baseball-fact source of truth.

Request fingerprints are deterministic over operation identity, target game, target at-bat, submission family, accepted result classification, accepted outcome reference, and sorted request facts supplied by the caller. The fingerprint boundary does not use current time, SwiftUI state, display strings, memory addresses, or unordered traversal. Task 7.7 still owns the production presentation/workflow operation identity and exact request-fact ownership.

`LegacyScoringOperationEvidenceAdapter` provides the non-routed persistence boundary for Task 7.7. It classifies existing evidence before mutation, treats matching persisted evidence as exact retry, treats mismatched identity/fingerprint/target evidence as conflict, and saves a supplied Legacy mutation plus the new evidence record with one `ModelContext.save()`. Save failure rolls back the context and reconciles through persisted evidence before declaring retry safe. Conflicting reuse does not call the supplied mutation. Exact retry returns the existing accepted evidence without reapplying the Legacy mutation.

`LegacyScoringOperationEvidenceFreshLookup` is the read-only fresh-context reconciliation owner. It creates a new context against the same container, disables autosave, fetches by stable operation identity, compares the persisted fingerprint and targets with the submitted request, and returns exact accepted retry, conflict, no evidence, or lookup failure. It performs no writes and does not route production baseball reads through the lookup.

The migration plan is `ScoreKeepProposedLegacyScoringOperationEvidenceMigrationPlan`, a lightweight V3-to-V4 migration. Existing V3 stores migrate with all Legacy models, all five canonical scoring models, and team-creation operation evidence preserved. Existing stores have zero Legacy scoring-operation evidence after migration unless future accepted operations create records through the authorized Legacy writer boundary. The migration does not synthesize operation evidence for historical Legacy scoring, does not perform canonical backfill, and does not retire Legacy persistence.

The Task 7.7B completion correction makes V4 the current production persistence schema and container target. V3 remains the frozen source and compatibility schema. Production container selection is explicit V4 by default, supported V3 stores migrate to V4 through copied-workspace migration with destination metadata verification, existing V4 stores open directly, unsupported source states fail closed, and recovery continues to preserve source and rollback evidence. Focused verification covers schema inventory, V3-to-V4 migration with zero historical operation-evidence backfill, operation-evidence round-trip, request fingerprint round-trip, atomic Legacy mutation plus evidence, injected save failure, exact retry, conflicting fingerprint, fresh-context accepted/conflict/no-evidence lookup, read-only lookup behavior, and ordinary live-scoring non-routing. Production scoring remains Legacy. Task 7.7B did not route ordinary scoring to Legacy scoring-operation evidence and ordinary scoring performed no canonical writes. Task 7.7 was unblocked but not started at Task 7.7B completion, and Task 7.8 had not begun.

<!-- MARK: - 41. Task 7.7 Duplicate Prevention Integration Record -->
## 41. Task 7.7 Duplicate Prevention Integration Record

Task 7.7 integrates the Task 7.7B Legacy scoring-operation evidence foundation into the live-scoring submission boundary. The operation-identity owner is the Legacy application-service submission boundary. Ordinary scoring receives one stable value-only identity for the selected game, target at-bat, ordinary family, and result before the first persistence attempt. Additional-choice scoring stores one stable value-only identity on `PendingAdditionalScoringChoice` from preparation through final submission. The supported transient lifetime is repeated callbacks, retry after confirmed failure, retry after uncertain save reconciliation, exact retry after accepted completion when the caller still supplies the identity, and presentation/coordinator reconstruction when the pending value identity is preserved. App-relaunch reconstruction of an unpersisted pending identity remains unsupported by this task.

Ordinary fingerprints include operation identity, target game, target at-bat, ordinary submission family, selected Legacy result, accepted Legacy at-bat outcome reference, target batter identity, batting team, target at-bat order, column, sequence, initial result, prepared current batter, prepared current pitcher, batting side, inning, outs, and the ordinary default boundary. Additional-choice fingerprints include those shared facts plus the completed choice facts for result, maximum base, base-path out, RBI, stolen bases, earned-run status, play-record evidence, sacrifice facts, and additional-choice category. Both boundaries use deterministic sorted request facts and exclude UI labels, navigation state, sheet lifecycle, current time, memory addresses, unordered traversal, and mutable post-operation projections.

`LiveScoringWorkflowCoordinator` performs the Task 7.7 authoritative flow below presentation when a V4 `ModelContext`, its `ModelContainer`, and `LegacyScoringOperationEvidenceAdapter` are supplied. It rebuilds current Legacy evidence, revalidates prepared state and enabled action state, builds the request fingerprint, asks the adapter to classify existing evidence, returns duplicate-prevented for exact accepted retry, fails closed for conflicting identity reuse, and saves the Legacy mutation plus accepted evidence atomically for a new operation. A thrown or completion-uncertain save is reconciled by `LegacyScoringOperationEvidenceFreshLookup`, which owns read-only fresh-context lookup against the same V4 store. Exact persisted evidence returns the existing accepted outcome; conflicting evidence fails closed; no accepted evidence permits retry with the same identity.

Cancellation, nil pending choice submission, unsupported choices, disabled actions, stale targets, and validation rejection do not persist accepted Legacy operation evidence. Conflict does not mutate Legacy facts or overwrite evidence. Existing scoring vocabulary, baseball calculations, additional-choice behavior, correction/deletion/replacement/substitution behavior, reports, exports, purchases, startup recovery, V3 compatibility, and XCTest launch isolation remain unchanged except for accepted ordinary and additional-choice scoring now writing Legacy operation evidence atomically with the Legacy mutation.

Production scoring remains Legacy. Accepted ordinary and additional-choice scoring writes only Legacy baseball facts and Legacy scoring-operation evidence. It does not invoke `CanonicalScoringTransactionAdapter`, does not write canonical scoring records, does not use canonical replay as the production reader, does not synthesize historical Legacy operation-evidence records, does not synthesize canonical history, does not change scoring authority, does not retire Legacy persistence, and does not begin Task 7.8. Task 7.8 is unblocked by this completion but remains not started.
