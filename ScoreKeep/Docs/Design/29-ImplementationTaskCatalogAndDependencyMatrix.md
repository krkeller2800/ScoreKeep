...
| --- | --- | --- | --- | --- | --- |
| 11.1 Retire legacy scoring authority | Remove or isolate obsolete scoring writer/calculation paths. | Phase 7 production routing and scoring evidence. | Retires legacy scoring authority. | No active route writes legacy scoring; reports/exports still pass. | cleanup |
...
| Live scoring | Internal routing, long session, device, resume, duplicate, accessibility evidence pass. | Explicit production routing approval. | Phase 11 live-scoring retirement. |
...
5. Phase 5 application-service coordination for game setup, game creation, live scoring, correction, substitution, and resume.
6. Phase 6 live-scoring shell and navigation preparation.
7. Phase 7 internal live-scoring routing, production live-scoring routing, and cutover gates.
8. Phase 4 migration and compatibility acceptance for import/export and file-open routes, where not already completed earlier.
9. Phase 9 purchase and allowance cutover for game creation, MLB downloads, paywall interruption, and restore/status behavior.
10. Phase 8 generated-output semantic acceptance and routing.
11. Phase 10 accessibility consolidation across routed workflows.
12. Phase 11 legacy retirement and cleanup.
13. Phase 12 release-candidate acceptance and release recommendation.

This path refines Document 28 by making live scoring depend on domain, scoring, persistence, application-service coordination, presentation shell, and cutover gates. Compatibility fixture work, purchase verification, generated-output semantic comparison, and accessibility review can start earlier, but they still block final release if unresolved.
...
Recommended first implementation task: `0.5 Compatibility route inventory`.

This is the safest first task because repository evidence confirms compatibility routes are central to later fixture work, migration, seeded data, import/export replacement, file-open routing, and release acceptance, while the current repository has only one checked-in `.ScoreKeep_Games` fixture and no checked-in `.ScoreKeep_Players` fixture found during inspection. Completing the route inventory before fixture creation reduces the risk of placing fixtures in the wrong context, missing a file-open path, or creating tests that cover only one entry point.

The task may inspect `ScoreKeep/Info.plist`, `ScoreKeep/Common/Extensions.swift`, `ScoreKeep/ScoreKeepApp.swift`, `ScoreKeep/List Data/StartView.swift`, `ScoreKeep/List Data/StartPhoneView.swift`, `ScoreKeep/Sharing Data/ImportService.swift`, `ScoreKeep/Sharing Data/ImportPlayersView.swift`, `ScoreKeep/Sharing Data/ShareContentView.swift`, `ScoreKeep/Sharing Data/DownloadFiles.swift`, `ScoreKeep/Seed/seededGame.ScoreKeep_Games`, `ScoreKeep/Docs/ScoreKeep-URL-Inventory.md`, the acceptance fixture catalog, and existing tests.

The task may change only documentation created for the inventory if a documentation location is explicitly chosen in the prompt. It must not change production source, tests, fixtures, seeded data, StoreKit configuration, project settings, schemes, build configurations, existing design documents unless specifically approved, import/export files, generated reports, Git configuration, or files inside `.git`.

Verification should confirm the branch, working tree, route evidence inspected, no unsupported repository claims, no source changes unless explicitly allowed, and a reviewed diff. If the task creates a documentation artifact, it should build only if the prompt requires it; otherwise a no-build rationale is acceptable because the task is documentation-only.

Recommended following task: `0.9 Representative .ScoreKeep_Players fixture curation`, after `0.5` identifies the supported roster import/export/download/open routes and any fixture-location decision is approved.
