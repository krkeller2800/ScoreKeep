...
The stabilization boundary is fail closed at `semanticVerifierUnavailable.currentTargetV2AndV3DuplicateEffectiveChecksums`. Normal production startup may open V3 directly or create a new empty V3 store where authorized, but it must not construct V2 and V3 together or silently skip required recovery verification. Future resolution requires one separately approved architectural decision: create a V2-only verification target for test proof, provide an authoritative frozen Core Data V2 model, or redesign the recovery-verification requirement. Until then, production scoring remains Legacy and Task 3.23 remains blocked.
...
| Phase 3 canonical scoring persistence | 3.21 design before 3.22 schema/storage, 3.22 before 3.23 adapter, 3.23 before 3.24 replay verification, 3.24 before 3.25 rehearsal | Non-routed presentation and report projection work that does not write scoring facts | Canonical scoring event, operation, correction, supersession, idempotency, and replay persistence | Versioned storage, one-save adapter, relaunch replay, malformed fail-closed, disposable rehearsal | 2.21 renewed scoring readiness, then 7.21 bounded production scoring routing | 11.1 scoring retirement only after routed proof for every affected operation |
...
Canonical scoring persistence design, implementation, adapter, persisted replay verification, disposable rehearsal, renewed scoring readiness, production routing, and Legacy scoring retirement must remain ordered as 3.21, 3.22, 3.23, 3.24, 3.25, 2.21, 7.21, and 11.1 for affected scoring operations. Task 3.20 may retire only obsolete persistence adapters with replacement and migration evidence; it does not authorize canonical scoring persistence or Legacy scoring removal.
