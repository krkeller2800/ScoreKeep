...
Scorecard columns are presentation evidence and not the sole authoritative event order. Legacy columns can help reconstruct how a scorecard was displayed, but they cannot safely decide baseball chronology when they conflict with sequence, inning, half inning, or correction state. Ambiguous ordering should produce warnings or repair requirements rather than silent guessing. Legacy seq is scorecard-presentation evidence and is not automatically a global canonical event sequence. Repeated or conflicting Legacy sequence values must remain visible as contradiction or repair-required evidence. Source array position, source index, scorecard column, and compound presentation coordinates must not silently replace conflicting canonical order evidence. A verification or migration adapter must not synthesize accepted chronology unless a separately approved repair policy defines the source, precedence, diagnostics, and audit trail. A Legacy artifact with unresolved ordering contradiction may be used to verify fail-closed behavior but not accepted replay correctness.
