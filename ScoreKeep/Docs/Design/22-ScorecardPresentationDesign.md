...
The substitution review is presented with the current sheet/navigation conventions and remains value-only until confirmation. Confirmation delegates through `LiveScoringShellPresentation.confirmSubstitutionReview` and `confirmPitcherChangeReview` to `LiveScoringWorkflowCoordinator.submitSubstitution` and `submitPitcherChange`; cancellation dismisses without workflow submission. Accepted results refresh the live scorecard from the authoritative Legacy workflow result and the shell's existing refresh path. Game-missing, participant-missing, wrong-team, duplicate, unsupported, rejected, or failed outcomes are not accepted and preserve the previously accepted presentation and persisted state. Historical pitcher editing and roster management remain separated and unchanged, durable substitution idempotency remains unsupported, production substitution remains Legacy, canonical substitution services remain non-routed, and Task 7.11 has not begun.
