...
Pregame drag-and-drop lineup preparation remains available before game creation/scoring through Team management's Default Batting Order screen. Saving there updates the Team default order used by the next newly materialized game and future games. What was retired is the separate game-specific Starting Lineup editor that could maintain a pregame order independently of the Team default.
The live scorecard is the primary wrong-Player correction surface. For editable slots it shows the full same-Team roster, safely reassigns unassigned Players, and swaps with another occupied editable slot when needed. Team management's Default Batting Order screen is the full roster/default-order editor and updates the Team default order for future games.
