...
Users should be able to add players from the team roster view, during pregame preparation, substitution preparation, and from supported import or paste workflows. Ordinary Team to Players roster add uses the shared player draft form for name, number, position, batting direction, batting order, team association, and photo selection or paste, and it creates no placeholder player before Save. Standard iPad Add Player uses a shared page-style sheet presentation across roster, substitution, pitcher, and lineup entry points, while iPhone keeps the existing compact phone presentation. The substitution and lineup screens should use toolbar Add Player routes instead of inline quick-entry rows. Adding a player should require only the information necessary to identify and use the player, with optional details available when the user has time.
