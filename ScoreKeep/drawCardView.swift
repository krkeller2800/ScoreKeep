    ...
    var body: some View {
        ...
        ForEach(Array(atbats.enumerated()), id: \.1) { index, atbat in
            if let xx = com.battings.firstIndex(where: { $0 == atbat.result }) {
                ...
                let y = (atbat.batOrder - yoffset) * space.height + space.minY
                let w = space.width
                let h = space.height
                let outs = atbat.outs
                ...
                if outs == 1 && com.outresults.contains(atbat.result) {
                    Text("*")
                        .font(.system(size: 18)).bold().foregroundColor(.red)
                        .position(x: 0.1 * w + x, y: 0.2 * h + y)
                } else if outs == 2  && com.outresults.contains(atbat.result) {
                    Text("**")
                        .font(.system(size: 18)).bold().foregroundColor(.red)
                        .position(x: 0.15 * w + x, y: 0.2 * h + y)
                } else if outs == 3  && com.outresults.contains(atbat.result) {
                    Path() {
                        myPath in
