    ...
    var body: some View {
        ZStack {
            ...
            HStack {
                ...
                Button("Add Player") {
                    ...
                    thisPlayer = Player(name: "", number: "", position: "", batDir: "", batOrder: 99, team: team)
        ...
        List {
            Section {
                HStack (spacing:0) {
                    Text("Name")
                        .frame(width:125).border(.gray).foregroundColor(.red).bold().background(.yellow.opacity(0.3))
                    Spacer()
                    Text("Num")
                ...
                ForEach(linePlayers) { player in
                        HStack {
                            Text(player.name).frame(width: 125, alignment: .leading).foregroundColor(.black).bold().lineLimit(1).minimumScaleFactor(0.5)
                            ...
                            Text(player.batDir).frame(maxWidth: .infinity, alignment: .leading).foregroundColor(.black).bold()
                                .overlay(Divider().background(.black), alignment: .trailing).minimumScaleFactor(0.2).padding(.trailing,5)
                            Spacer()
                            Text("\(player.batOrder)").frame(maxWidth: .infinity, alignment: .center).foregroundColor(.black).bold()
