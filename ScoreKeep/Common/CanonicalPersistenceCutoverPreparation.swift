...
enum CanonicalPersistenceCutoverRouteManifest {
    static let requiredProductionWriterReferences: Set<String> = [
        ...
        "PlayersOnTeamView.headerAddPlayerAndRosterInsertDeleteOrder",
        "EditPlayerView.playerFieldsAndPhoto",
        "LineupSlotSafetyCoordinator.materializeLineupIfNeeded",
        "LineupSlotSafetyCoordinator.reassignPlayer",
        "LineupSlotSafetyCoordinator.swapPlayers",
        "TeamDefaultBattingOrderView.save",
    ...
    static let routes: [CanonicalPersistenceCutoverRouteAssessment] = [
        ...
        route(.playerCreationAndEditing, "User creates or edits reusable player records.", ["PlayerView.playerInsertDeleteOrder", "PlayersOnTeamView.headerAddPlayerAndRosterInsertDeleteOrder", "EditPlayerView.playerFieldsAndPhoto", "PitcherContentView.addPlayer"], read: ["Player", "Team", "Atbat", "Pitcher"], inserted: ["Player"], updated: ["Player"], deleted: ["Player"], relationships: ["Player.team", "Team.players"], ordering: ["Player.batOrder renumbering"], media: ["Player.photo"], explicit: "Creation and some deletes save explicitly.", implicit: "Field, order, team, and photo edits often rely on bound model persistence.", failure: "Some errors alert; many are ignored or logged.", user: "Player appears, changes, is deleted, or reference warning is shown.", purchase: "None.", current: .legacySwiftData, future: .proposedPersistenceAuthority, migration: true, mapping: true, oneWriter: true, independent: true, disable: .schemaIndependentDisablePossible, rollback: .recoveryOnlyUntilBackupDesigned, verify: ["player identity", "roster relationship", "batting-order verification", "media behavior"], retire: ["player create/edit/delete replacement verified"], classes: [.legacyWriter, .mediaWriter, .blockedByMissingRollback]),
        route(.rosterChanges, "User edits team roster membership and batting order.", ["PlayersOnTeamView.headerAddPlayerAndRosterInsertDeleteOrder", "PlayerView.playerInsertDeleteOrder", "PasteView.importPlayers"], read: ["Team", "Player", "Atbat", "Pitcher"], inserted: ["Player"], updated: ["Player.team", "Player.batOrder", "Player display fields"], deleted: ["Player"], relationships: ["Team.players", "Player.team"], ordering: ["Player.batOrder"], media: ["Player.photo through import or edit"], explicit: "Several insert/delete paths save explicitly.", implicit: "Order and relationship edits can be implicit.", failure: "No coherent transaction across roster and order updates.", user: "Roster list updates or deletion warning appears.", purchase: "None.", current: .legacySwiftData, future: .proposedPersistenceAuthority, migration: true, mapping: true, oneWriter: true, independent: false, disable: .blockedUntilDesigned, rollback: .blockedUntilDesigned, verify: ["roster relationship", "ordering", "duplicate prevention"], retire: ["all roster mutation paths replaced"], classes: [.legacyWriter, .blockedByMissingRollback]),
        route(.lineupChanges, "User materializes a game lineup from the Team default, safely corrects a scorecard lineup slot, swaps two eligible scorecard slots, or saves Team default batting order.", ["LineupSlotSafetyCoordinator.materializeLineupIfNeeded", "LineupSlotSafetyCoordinator.reassignPlayer", "LineupSlotSafetyCoordinator.swapPlayers", "TeamDefaultBattingOrderView.save"], read: ["Game", "Team", "Player", "Lineup", "Atbat"], inserted: ["Lineup", "Atbat placeholders"], updated: ["Lineup.players", "Player.batOrder", "Atbat.player"], deleted: [], relationships: ["Game.lineups", "Lineup.players", "Game.atbats", "Game.players"], ordering: ["lineup order", "Team default batting order", "event sequence"], media: [], explicit: "Coordinator and Team default saves occur through bounded operations.", implicit: "SwiftData relationship persistence still stores the current model graph.", failure: "Ambiguous or participated game lineup state fails closed instead of destructive repair.", user: "Default order saves or scorecard identity correction appears immediately.", purchase: "None.", current: .legacySwiftData, future: .proposedPersistenceAuthority, migration: true, mapping: true, oneWriter: true, independent: false, disable: .blockedUntilDesigned, rollback: .blockedUntilDesigned, verify: ["lineup relationship", "event preservation", "Team default order", "safe slot correction"], retire: ["lineup adapter verified"], classes: [.legacyWriter, .blockedByMissingRollback, .blockedByMissingUserReview]),
        ...
        route(.playerDeletion, "User deletes a player from roster/list/paste workflows.", ["PlayerView.playerInsertDeleteOrder", "PlayersOnTeamView.headerAddPlayerAndRosterInsertDeleteOrder", "PasteView.deleteAllPlayersOnSelectedTeam"], read: ["Player", "Atbat", "Pitcher", "Team"], inserted: [], updated: ["remaining player batting order"], deleted: ["Player"], relationships: ["Team.players", "Player.team"], ordering: ["remaining batting order"], media: ["Player.photo removed with owner"], explicit: "Some deletes save explicitly; some do not until later.", implicit: "Relationship delete effects are implicit.", failure: "Reference checks are name-based in places and not transaction-classified.", user: "Player disappears or reference warning appears.", purchase: "None.", current: .legacySwiftData, future: .proposedPersistenceAuthority, migration: true, mapping: true, oneWriter: true, independent: false, disable: .blockedUntilDesigned, rollback: .blockedUntilDesigned, verify: ["reference checks", "ordering", "orphan review"], retire: ["player delete adapter verified"], classes: [.legacyDestructiveWriter, .deleteOrCleanupWriter, .blockedByMissingRollback, .blockedByMissingUserReview]),
        route(.eventDeletion, "User deletes or clears an at-bat event.", ["ScoreGameView.eventDeletion"], read: ["Atbat", "Game"], inserted: [], updated: ["Atbat reset fields", "Game.atbats"], deleted: ["Atbat"], relationships: ["Game.atbats"], ordering: ["event sequence after deletion"], media: [], explicit: "Delete may occur on disappear or during scorecard correction.", implicit: "Field reset is bound mutation.", failure: "No supersession, rollback, or proof of downstream projection repair.", user: "Scoring event clears or disappears.", purchase: "None.", current: .legacySwiftData, future: .proposedPersistenceAuthority, migration: true, mapping: true, oneWriter: true, independent: false, disable: .blockedUntilDesigned, rollback: .blockedUntilDesigned, verify: ["correction policy", "projection recompute", "rollback"], retire: ["event delete adapter verified"], classes: [.legacyDestructiveWriter, .deleteOrCleanupWriter, .mustRemainLegacyUntilLater, .blockedByUnsupportedCanonicalMeaning, .blockedByMissingUserReview]),
