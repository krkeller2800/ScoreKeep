...
//            .position(x: from.minX + (space.width * 2.5), y:space.minY - 7)
        if atbats.first?.persistentModelID == atbat.persistentModelID {
            ...
            Text("R")
                ...
                .position(x: space.minX + (1.25 * space.width), y: 0.8 * space.height + newy)
            Text("H")
                .font(.system(size: 10, weight: .semibold)).foregroundColor(.black)
                .position(x: space.minX + (1.75 * space.width), y: 0.8 * space.height + newy)
        }
        Rectangle()
            .fill(Color.white)
...
struct drawBoxScore: View {
    ...
    var body: some View {
        ...
        Text(UIDevice.type == "iPhone" ? "  Inn  R  H  E" : "Inning  Runs   Hits   Errors")
            .font(.caption).italic().foregroundStyle(ScoreKeepVisualStyle.primaryText).frame(width:250,alignment: .leading)
            .position(x: placeBox.minX + 125, y:placeBox.minY - 10 + 2*adj)
        Text(theHalf)
