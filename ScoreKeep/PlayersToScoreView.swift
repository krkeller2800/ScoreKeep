...
struct PlayersToScoreView: View {
    ...
    @State var theAtbat = Atbat(game: Game(date: "", location: "", highLights: "", vscore: 0, hscore: 0),
                                team: Team(name: "", coach: "", details: ""),
                                player: Player(name: "", number: "", position: "", batOrder: 99),
                    ...
                    ZStack {
                        VStack (spacing: 0) {
                            HStack {
                                ...
                                Text("Name").frame(maxWidth:70, alignment:.center).font(.caption).foregroundColor(.black).bold()
                                Spacer()
                            }
                            ForEach(Array(atbats.enumerated()), id: \.1) { index, atbat in
                                HStack(spacing: 0) {
                                    if atbat.inning <= 1 && atbat.col == 1 && atbat.batOrder != 99 {
                                        ...
                                        Text(atbat.player.name).frame(maxWidth: 90, alignment: .leading).foregroundColor(.black).bold()
                                        ...
                                        ForEach((1...maxCol), id: \.self) {_ in
                                            ...
                                            .simultaneousGesture(DragGesture(minimumDistance: 0, coordinateSpace: .global)
                                                .onEnded {
                                                    ...
                                                    let offset = UIScreen.main.bounds.size.width - geometry.size.width + 124.0
                                                    ...
                                                    if let curratbat = atbats.firstIndex(where: {  $0.game == atbat.game &&
                                                        $0.team == atbat.team &&
                                                        $0.batOrder == CGFloat(posy) &&
                                                        $0.col == Int(posx) })
                                                    {
                                                        theAtbat = atbats[curratbat]
                                                    } else {
                                                        let newAtbat = Atbat(game: atbat.game, team: atbat.team, player: atbat.player, result: "Result",
                                                                             maxbase: "No Bases", batOrder: CGFloat(posy), outAt: "Not Out", inning: 99, seq:99, col: Int(posx), rbis:0, outs:0, sacFly: 0,sacBunt: 0,stolenBases: 0)
                        ...
                        let newx:CGFloat = UIScreen.screenWidth > 1300 ? 65 : 75
        ...
        for (_, atbat) in atbatsSoFar.enumerated() {
            if atbat.result != "Result" {
                ...
                if atbat.maxbase == "Home" {
                    colbox[col].runs += 1
                    batbox[Int(atbat.batOrder)].runs += 1
                    totbox[0].runs += 1
                }
                if atbat.result == "Home Run" {
                    colbox[col].HR += 1
                    batbox[Int(atbat.batOrder)].HR += 1
                    totbox[0].HR += 1
                }
                if com.hitresults.contains(atbat.result) {
                    colbox[col].hits += 1
                    batbox[Int(atbat.batOrder)].hits += 1
                    totbox[0].hits += 1
                }
                if atbat.result == "Walk" {
                    colbox[col].walks += 1
                    batbox[Int(atbat.batOrder)].walks += 1
                    totbox[0].walks += 1
                }
                if atbat.result == "Strikeout" || atbat.result == "Strikeout Looking" {
                    colbox[col].strikeouts += 1
                    batbox[Int(atbat.batOrder)].strikeouts += 1
