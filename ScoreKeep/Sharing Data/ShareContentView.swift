...
struct ShareContentView: View {
                ...
                VStack {
                    ...
                    HStack {
                        ...
                        VStack {
                            ...
                            .border(.gray).cornerRadius(10).accentColor(.black)
                            .opacity(doShare == "Team" ? 0 : 1)
                            .allowsHitTesting(doShare == "Team" ? false : true)
                        }
                        Spacer()
                    }
    ...
    func getPlayers (players: [Player]) -> [SharePlayer] {
        ...
        for player in players {
            if player.team != nil {
                let sharePlayer = SharePlayer(name: player.name, number: player.number, position: player.position, batDir: player.batDir, batOrder: player.batOrder,team: getTempTeam(team: player.team!),
                                              photo: player.photo ?? Data())
                sharePlayers.append(sharePlayer)
            }
        }
        return sharePlayers
